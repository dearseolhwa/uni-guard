...
alter table public.barangays add column if not exists city   text not null default 'Dagupan City';
...
alter table public.advisories add column if not exists affected_area text not null default 'Citywide';
...
alter table public.emergency_hotlines add column if not exists scope          text not null default 'Citywide';
