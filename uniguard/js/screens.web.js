...
const UG_WEB = (() => {
  ...
  const ROLE_INFO = {
    citizen:           { key:'citizen',           label:'Citizen',            area:'citizen', home:'home',      scope:(b) => 'Barangay ' + (b || 'Bonuan Gueset') },
    barangay_official: { key:'barangay_official', label:'Barangay Official',  area:'console', home:'dashboard', scope:(b) => 'Barangay ' + (b || 'Bonuan Gueset') },
    lgu_ldrrmc:        { key:'lgu_ldrrmc',        label:'LGU / LDRRMC',       area:'console', home:'dashboard', scope:() => 'Citywide · Dagupan' },
  };
  ...
  function cwTop(st) {
    ...
    const active = st.route === 'advisory-detail' ? 'advisories' : (st.route === 'report-detail' ? 'notifications' : st.route);
    return '<header class="ug-wtop">' +
      ...
      '<nav class="ug-wnav">' + CNAV.map(n =>
        ...
        I(n[2], 16) + n[1] + (n[0] === 'notifications' && unread ? '<span class="ug-wdot">' + unread + '</span>' : '') + '</button>').join('') +
      '</nav>' +
  ...
  function cwHome(st) {
    ...
    return '<div class="ug-col" style="gap:18px">' +
      cwHead('Home', 'Live situation for Barangay Bonuan Gueset, Dagupan City.',
          ...
          '<div class="ug-card"><div class="ug-card-h"><h3>Emergency Hotlines</h3>' +
            ...
            '<div class="ug-rows">' + U.DATA.hotlines.slice(0, 4).map(h =>
              '<div class="ug-row" style="padding:10px 16px"><div class="r-main">' +
                ...
                '<button class="ug-btn ug-btn--sm"' + A('toast', attr({ msg: 'Calling ' + h.agency, tone: 'prepared' })) + '>' + I('phone', 13) + '</button></div>').join('') + '</div></div>' +
        ...
        '<div class="ug-col" style="gap:18px">' +
          ...
          '<div class="ug-card"><div class="ug-card-h"><h3>Incident Coordinates</h3></div>' +
            '<div class="ug-card-b"><div class="ug-geo"><span class="geo-ico">' + I('pin', 18) + '</span>' +
              ...
              '<div class="ug-rowf ug-between" style="font-size:12px"><span class="ug-dim">Barangay queue</span><span class="ug-mono">Bonuan Gueset</span></div>' +
    ...
    return '<div class="ug-col" style="gap:18px">' +
      ...
      '<div class="ug-wgrid-article">' +
        ...
        '<div class="ug-col" style="gap:18px;min-width:0">' +
          ...
          '<div class="ug-card ug-card--flat"><div class="ug-card-b ug-col" style="gap:9px">' +
            '<button class="ug-btn ug-btn--sm ug-btn--block"' + A('toast', attr({ msg: 'Advisory saved for offline reading', tone: 'info' })) + '>' + I('download', 15) + 'Save Offline</button>' +
            '<button class="ug-btn ug-btn--sm ug-btn--block ug-btn--ghost"' + A('toast', attr({ msg: 'Link copied to clipboard', tone: 'info' })) + '>Share</button>' +
          '</div></div>' +
        '</div>' +
      '</div>' +
    '</div>';
      ...
      '<div class="ug-wgrid-cards">' + list.map(c =>
        '<div class="ug-card"><div class="ug-card-b ug-col" style="gap:11px">' +
          ...
          '<div class="ug-rowf ug-gap8" style="gap:8px">' +
            '<button class="ug-btn ug-btn--sm" style="flex:1"' + A('toast', attr({ msg: 'Route to ' + c.name + ' opened', tone: 'info' })) + '>' + I('route', 14) + 'Directions</button>' +
            '<button class="ug-btn ug-btn--sm ug-btn--ghost"' + A('toast', attr({ msg: c.name + ' saved offline', tone: 'info' })) + '>' + I('download', 14) + '</button></div>' +
        '</div></div>').join('') + '</div>' +
    '</div>';
  }
  ...
  function cwHotlines(st) {
    return '<div class="ug-col" style="gap:18px">' +
      cwHead('Hotlines', 'Emergency Hotline Directory for barangay and city offices.',
        '<button class="ug-btn ug-btn--ghost ug-btn--sm"' + A('toast', attr({ msg: 'Hotline list saved offline', tone: 'prepared' })) + '>' + I('download', 15) + 'Save Offline</button>') +
      '<div class="ug-wgrid-hotlines">' + U.DATA.hotlines.map(h =>
        '<div class="ug-card"><div class="ug-card-b ug-col" style="gap:10px">' +
          ...
          '<button class="ug-btn ug-btn--sm ug-btn--signal"' + A('toast', attr({ msg: 'Calling ' + h.agency, tone: 'prepared' })) + '>' + I('phone', 14) + 'Call</button>' +
        '</div></div>').join('') + '</div>' +
