...
const Repo = (function () {
  ...
  /* ------------------------------------------------------- shape adaptation */
  const CITY_BOUNDS = { minLat: 16.005, maxLat: 16.075, minLng: 120.290, maxLng: 120.365 };
  ...
  function recomputeKpis() {
    ...
    const active = (c.openIncidents != null) ? c.openIncidents : inc.filter((i) => i.status !== 'resolved').length;
    const critical = (c.emergency != null) ? c.emergency : inc.filter((i) => i.sev === 'emergency' && i.status !== 'resolved').length;
  ...
  async function createReport(input) {
    ...
    if (!c) {
      ...
      const row = toIncident({
        code: code, hazard_type: input.hazard_type, barangay: input.barangay, description: input.description,
  ...
  async function createAdvisory(input) {
    ...
    const { data, error } = await c.from('advisories').insert({
      ...
      affected_area: input.affected_area || (input.citywide ? 'Citywide' : 'Selected barangays'),
  ...
  async function createCenter(input) {
    const c = client();
    if (!c) {
      UG.DATA.centers.push(toCenter({ id: 'local-' + Date.now(), name: input.name, barangay: input.barangay, capacity: input.capacity, occupancy: 0, status: 'open' }));
      emit();
      return { local: true };
    }
  ...
  async function declareEmergency(payload) {
    ...
    const { data, error } = await c.rpc('declare_emergency', {
      p_title: (payload && payload.title) || 'Emergency declaration: citywide response activated',
      p_body: (payload && payload.body) || '',
      p_area: (payload && payload.area) || 'Citywide'
    });
    if (error) fail(error);
    await loadAll();
      ...
      try {
        await c.functions.invoke('push-dispatch', {
          body: { title: (payload && payload.title) || 'Emergency declaration', body: (payload && payload.body) || '',
                  severity: 'emergency', advisoryId: data.advisory_id, area: (payload && payload.area) || 'Citywide' }
        });
