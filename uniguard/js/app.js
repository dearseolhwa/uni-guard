...
(function () {
  ...
  const S = {
    ...
    draft: { title: '', sev: 'warning', type: 'Emergency', area: 'Citywide', msg: '' },
    report: {
      hazard: 'Flash Flood', brgy: 'Bonuan Gueset', urg: 'warning', desc: '',
  ...
  const APP_ACTIONS = {
      ...
      try {
        if (!UG_PWA.state.online || !Repo.online()) {
          await UG_PWA.enqueue({
            hazard_type: r.hazard, barangay: r.brgy, description: r.desc.trim(),
        ...
        S.route = 'report-done';
        S.report = { hazard: r.hazard, brgy: r.brgy, urg: 'warning', desc: '', photo: false, photoBlob: null, photoName: '', gps: false, lat: null, lng: null, accuracy: null, error: '', busy: false };
      } catch (e) {
        r.busy = false;
    ...
    'add-center': async () => {
      ...
      try {
        await Repo.createCenter({ name: name.trim(), barangay: brgy, barangay_id: brgyRow ? brgyRow.id : null, capacity: cap, occupancy: 0, status: 'open' });
        toast('Evacuation centre added to the directory', 'prepared');
      } catch (e) { toast(e.message, 'warning'); }
      render();
    },
    ...
    'add-hotline': async () => {
      ...
      const scope = ((q('[data-field="hotlineScope"]') || {}).value || '').trim() || 'Citywide';
    ...
    declare: async () => {
      ...
      const ok = await UG_FEATURES.confirm({
        ...
        message: 'Broadcast a citywide emergency alert to every subscribed device?',
      ...
      if (!ok) return;
      try {
        const res = await Repo.declareEmergency({ title: 'Emergency declaration: citywide response activated' });
    ...
    publish: async () => {
      ...
      try {
        await Repo.createAdvisory({
          ...
          affected_area: S.draft.area, citywide: /citywide/i.test(S.draft.area),
    ...
    'clear-draft': () => {
      S.draft = { title: '', sev: 'warning', type: 'Emergency', area: 'Citywide', msg: '' };
    ...
    directions: (d) => {
      ...
      if (c && typeof c.lat === 'number') {
        ...
        window.open('https://www.openstreetmap.org/search?query=' + encodeURIComponent((d.name || 'evacuation centre') + ' Dagupan City'), '_blank', 'noopener');
      }
      toast('Opening directions', 'info');
    },
    ...
    'admin-create': async () => {
      ...
      const email = await UG_FEATURES.prompt({ title: 'Create an official account', label: 'Email address', placeholder: 'official@dagupan.gov.ph' });
      if (!email) return;
      try {
